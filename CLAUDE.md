...
├── server/
│   ├── server.js                         ← Express API 서버 (Node.js, 포트 8765)
│   ├── package.json
│   └── public/
...
./start.sh    # 서버 시작 → http://localhost:8765
```

## 환경변수 (.env)
`server/.env` 파일로 설정. 인증 미설정 시 개발 모드(인증 없이 실행).

| 변수 | 설명 | 기본값 |
|------|------|--------|
| `BASIC_AUTH_USER` | 로그인 아이디 | — |
| `BASIC_AUTH_PASS` | 로그인 비밀번호 | — |
| `PORT` | 서버 포트 | `8765` |
| `ALLOWED_COUNTRIES` | 허용 국가 코드 (콤마 구분) | `KR` |

## 보안
- **Bearer 토큰 인증**: 로그인 시 발급, 모든 `/api/*` 요청에 포함
- **브루트포스 방어**: 실패 3~4회→5초 지연, 5~9회→30초 지연, 10회→IP 블랙리스트 영구 차단
- **블랙리스트**: `server/blacklist.json` 영구 보존. 해제 시 파일에서 IP 삭제 후 서버 재시작
- **지오블록**: `ALLOWED_COUNTRIES` 외 국가 IP의 `/api/*` 접근 403 차단 (로컬/사설 IP 제외)
- **인증 로그**: `server/auth.log` (LOGIN_SUCCESS / LOGIN_FAIL / LOGIN_BLACKLISTED / GEO_BLOCKED)
...
| PUT | `/api/events/:id` body: `{ startTime, endTime, endTimeNextDay, notes, title }` | 이벤트 수정 |
