...
| 백엔드 서버 | Node.js + Express (포트 8765) |
...
### 3. 환경변수 설정 (server/.env)
```
ALLOWED_COUNTRIES=KR        # 허용 국가 (콤마 구분, 기본값: KR)

BASIC_AUTH_USER=your_id
BASIC_AUTH_PASS=your_password
```
미설정 시 인증 없이 실행되는 개발 모드로 동작합니다.
...
- 로그인 인증 (Bearer 토큰)
- 브루트포스 방어: 실패 누적 시 응답 지연 → 10회 시 IP 영구 차단 (`server/blacklist.json`)
- 지오블록: `ALLOWED_COUNTRIES` 외 국가 IP 차단
...
PUT  /api/events/:id   { startTime, endTime, endTimeNextDay, notes, title }
DEL  /api/events/:id
